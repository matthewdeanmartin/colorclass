...
def test_list_tags():
    ...
    assert sorted(t for i in actual for t in i[:2] if t is not None) == sorted(
        BASE_CODES
    )
...
def test_disable_colors_piped(tty):
    ...
    command_colors_enabled = [
        sys.executable,
        "-c",
        assert_statement.format(bool="False"),
    ]
    command_colors_disabled = [
        sys.executable,
        "-c",
        assert_statement.format(bool="True"),
    ]

    # Run piped to this pytest process.
    if not tty:  # Outputs piped to non-terminal/non-tty. Colors disabled by default.
        proc = subprocess.Popen(
            command_colors_disabled, stderr=subprocess.STDOUT, stdout=subprocess.PIPE
        )
    ...
    if IS_WINDOWS:
        ...
        proc = subprocess.Popen(
            command_colors_enabled, close_fds=True, creationflags=c_flags
        )
    ...
    proc = subprocess.Popen(
        command_colors_enabled, stderr=subprocess.STDOUT, stdout=slave, close_fds=True
    )
    os.close(slave)
