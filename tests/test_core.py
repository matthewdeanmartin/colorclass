...
def test_apply_text():
    ...
    assert (
        apply_text("!\033[31mRed\033[0m", lambda s: s.upper()) == "!\033[31mRED\033[0m"
    )
    assert (
        apply_text("\033[1mA \033[31mB \033[32;41mC \033[0mD", lambda _: "")
        == "\033[1m\033[31m\033[32;41m\033[0m"
    )
...
def test_dunder(kind):
    ...
    assert_both(
        instance.__class__("1%s2" % instance),
        "1test ME 2",
        "1\033[31mtest ME \033[39m2",
    )
    assert_both(
        get_instance(kind, "1%s2") % "test ME ",
        "1test ME 2",
        "\033[31m1test ME 2\033[39m",
    )
    assert_both(
        get_instance(kind, "1%s2") % instance,
        "1test ME 2",
        "\033[31m1test ME \033[39m2",
    )

    assert_both(instance * 2, "test ME test ME ", "\033[31mtest ME test ME \033[39m")
    assert_both(
        instance + instance, "test ME test ME ", "\033[31mtest ME test ME \033[39m"
    )
    assert_both(instance + "more", "test ME more", "\033[31mtest ME \033[39mmore")
    assert_both(
        instance.__class__("more" + instance),
        "moretest ME ",
        "more\033[31mtest ME \033[39m",
    )
    instance *= 2
    assert_both(instance, "test ME test ME ", "\033[31mtest ME test ME \033[39m")
    instance += "more"
    assert_both(
        instance, "test ME test ME more", "\033[31mtest ME test ME \033[39mmore"
    )
...
def test_encode_decode(kind):
    ...
    if sys.version_info[0] == 2:
        ...
        assert_both(
            instance.__class__.decode(instance, "utf-8"),
            "test ME",
            "\033[31mtest ME\033[39m",
        )
    ...
    assert_both(
        instance.encode("utf-8").decode("utf-8"), "test ME", "\033[31mtest ME\033[39m"
    )
    assert_both(
        instance.__class__.encode(instance, "utf-8").decode("utf-8"),
        "test ME",
        "\033[31mtest ME\033[39m",
    )
...
def test_format(kind, mode):
    ...
    instance = get_instance(kind, "test me")
    assert_both(
        instance.__class__("1{0}2".format(instance)),
        "1test me2",
        "1\033[31mtest me\033[39m2",
    )
    assert_both(
        instance.__class__(str.format("1{0}2", instance)),
        "1test me2",
        "1\033[31mtest me\033[39m2",
    )

    # Get actual.
    template_pos = get_instance(
        kind, "a{0}c{0}", "bgred" if mode == "fg within bg" else "red"
    )
    template_kw = get_instance(
        kind, "a{value}c{value}", "bgred" if mode == "fg within bg" else "red"
    )
    ...
    assert_both(
        instance.__class__.format(template_pos, instance), expected[0], expected[1]
    )
    assert_both(
        instance.__class__.format(template_kw, value=instance), expected[0], expected[1]
    )
...
def test_format_mixed(kind):
    ...
    assert_both(
        instance.format("{blue}Moo{/blue}"),
        "XXX: {blue}Moo{/blue}",
        "\033[31mXXX: \033[39m{blue}Moo{/blue}",
    )
    assert_both(
        instance.format(get_instance(kind, "Moo", "blue")),
        "XXX: Moo",
        "\033[31mXXX: \033[34mMoo\033[39m",
    )
...
def test_j_s(kind):
    ...
    assert_both(
        instance.join(iterable),
        "Atest meB",
        "\033[32mA\033[31mtest me\033[32mB\033[39m",
    )
...
def test_t_z(kind):
    ...
    assert_both(
        get_instance(kind, "TEST YOU").title(), "Test You", "\033[31mTest You\033[39m"
    )

    table = {ord("t"): "1", ord("e"): "2", ord("s"): "3"}
    ...
    assert_both(
        get_instance(kind, "-10.3").zfill(6), "-010.3", "\033[31m-010.3\033[39m"
    )
...
def test_empty(kind):
    ...
    assert_both(
        instance.__class__.encode(instance, "utf-8").decode("utf-8"), "", "\033[39m"
    )
    ...
    assert_both(
        instance.translate({ord("t"): "1", ord("e"): "2", ord("s"): "3"}),
        "",
        "\033[39m",
    )
