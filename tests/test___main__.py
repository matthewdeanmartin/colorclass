...
def test_import_do_nothing():
    ...
    command = [
        sys.executable,
        "-c",
        "from colorclass.__main__ import TRUTHY; assert TRUTHY",
    ]
...
def test(monkeypatch, colors, light):
    ...
    command = [
        sys.executable,
        "-m",
        "colorclass" if sys.version_info >= (2, 7) else "colorclass.__main__",
    ]
    ...
    proc = subprocess.Popen(
        command, stderr=subprocess.STDOUT, stdout=subprocess.PIPE, stdin=subprocess.PIPE
    )
